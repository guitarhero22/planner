...
public class nAdapter extends RecyclerView.Adapter<nAdapter.ViewHolder> {
    ...
    public nAdapter(List<ListItem> cardList, Context context) {
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, int position) {
        ...
        holder.linearLayout.setOnClickListener(new DoubleClickListener(250) {
            @Override
            public void onDoubleClick() {
                Intent in = new Intent(context, lastlevel.class);
                in.putExtra( "TAG" , card.getTitle());
                datab database = datab.getInstance(context.getApplicationContext());
                List<table2> list = database.t2dao().getDes(card.getTitle());
                in.putExtra("Description", list.get(0).getDescription());
